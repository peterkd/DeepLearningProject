...
In this project, we focus on the most popular text dataset for Deep Learning sentiment analysis: the IMDB movie review set. Using this data, we build and train a recurrent LSTM autoencoder to compress reviews and extract the dense lower dimensional encoder layers for further training in an additional network for classifying sentiment. This approach achieved a test accuracy score of 82% after fine-tuning the encoder layers in the dense neural network classifier for sentiment. Training the sentiment classifier with frozen encoder layers was also tested but yielded only modest performance in accuracy.
